...
const findManyPrompt = async (args: {
  ...
  const promptMessage =
    `Find all documents in the MongoDB with cluster: ${connectionString} in database "${dbName}", collection "${collectionName}".` +
    (query
